...
namespace ygz
{
                                       ...
                                       vector<MapPoint *> &vpMatched, int th)
    {
        ...
        for (int iMP = 0, iendMP = vpPoints.size(); iMP < iendMP; iMP++)
        {
            ...
            // Get 3D Coords.
            cv::Mat p3Dw = Converter::toCvMat(pMP->GetWorldPos());
            ...
            cv::Mat Pn = Converter::toCvMat(pMP->GetNormal());
    ...
    int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
    {
        ...
        Vector3f Ow = pKF->GetCameraCenter();
                                       ...
                                       bool checkLevel)
    {
        ...
        const Vector3f tcw = CurrentFrame.mTcw.translation();

        const Vector3f twc = -1 * Rcw.transpose() * tcw;

        const Matrix3f Rlw = LastFrame.mTcw.rotationMatrix();
        const Vector3f tlw = LastFrame.mTcw.translation();

        const Vector3f tlc = Rlw * twc + tlw;
                                       ...
                                       const float th, const int ORBdist)
    {
        ...
        const Matrix3f Rcw = CurrentFrame.mTcw.rotationMatrix();
        const Vector3f tcw = CurrentFrame.mTcw.translation();
        const Vector3f Ow = -1 * Rcw.transpose() * tcw;
        ...
        MapPoint *mp, Vector2f &px_curr, int &search_level)
    {
        ...
        search_level = GetBestSearchLevel(ACR, ref->mvImagePyramid.size() - 1, ref);
        WarpAffine(ACR, ref->mvImagePyramid[kp.octave], px_ref, kp.octave, ref, search_level, WarpHalfPatchSize + 1,
                   _patch_with_border);
        ...
        bool success = ygz::Align2D(curr->mvImagePyramid[search_level], _patch_with_border, _patch, 10, px_scaled);
