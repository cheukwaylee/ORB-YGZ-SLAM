        ...
        : mnIterations(0), mnBestInliers(0), mbFixScale(bFixScale)
    {
        ...
        for (int i1 = 0; i1 < mN1; i1++)
        {
            if (vpMatched12[i1])
            {
                ...
                cv::Mat X3D1w = Converter::toCvMat(pMP1->GetWorldPos());
                mvX3Dc1.push_back(Rcw1 * X3D1w + tcw1);

                cv::Mat X3D2w = Converter::toCvMat(pMP2->GetWorldPos());
        ...
        mK1 = Converter::toCvMat(pKF1->mK);
        mK2 = Converter::toCvMat(pKF2->mK);
